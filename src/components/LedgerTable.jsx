...
// Maps column keys to their [asc, desc] sort values
const SORT_MAP = {
...
const TableRow = memo(function TableRow({ asset, even }) {
  ...
  return (
    <tr className={`border-b border-[#0f0f0f] hover:bg-[#151515] transition-colors duration-100
      ${even ? 'bg-[#0a0a0a]' : 'bg-[#080808]'}`}>
      <td className="px-4 py-3 font-mono text-[11px] text-[#bbb] whitespace-nowrap">{asset.id}</td>
      <td className="px-4 py-3 max-w-[200px]">
        <span className="text-sm text-white font-medium line-clamp-1 block">{asset.name}</span>
      </td>
      <td className="px-4 py-3 text-xs text-[#bbb] whitespace-nowrap hidden sm:table-cell">{asset.type}</td>
      ...
      <td className="px-4 py-3 text-xs text-[#bbb] whitespace-nowrap hidden lg:table-cell">{asset.country}</td>
      <td className="px-4 py-3 text-xs text-[#bbb] whitespace-nowrap hidden xl:table-cell font-mono">{asset.region}</td>
